...
class PagoBase(SQLModel):
    ...
    concepto: ConceptoPago   = Field(sa_column=Column(String, nullable=False))
    metodo_pago: MetodoPago  = Field(sa_column=Column(String, nullable=False))
    fecha_pago: datetime     = Field(default_factory=datetime.utcnow)
    estado: EstadoPago       = Field(sa_column=Column(String, nullable=False))
